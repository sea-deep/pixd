...
export default new Component({
  ...
  execute: async (interaction, client) => {
    ...
    const from = message.components[0].components[0].options.find(
      (piece: { default?: boolean }) => piece.default
    ).label;
    ...
    if (moved.captured) {
      ...
      let e = pieceEmoji[`${moved.captured.toLowerCase()}${turn}` as keyof typeof pieceEmoji];
    ...
    if (message.content.includes(client.user!.id)) {
      ...
      if (botMoved.captured) {
        let turn = chess.turn();
        let index = turn === "b" ? 1 : 2;
        let e = pieceEmoji[`${botMoved.captured.toLowerCase()}${turn}` as keyof typeof pieceEmoji];
        content[index] += `<:${e.name}:${e.id}>`;
        content = incrementScore(
          content.join("\n"),
          botMoved.color,
