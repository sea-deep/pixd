...
export default new Component({
  ...
  execute: async (interaction, client) => {
    ...
    let current = interaction.message.embeds[0].fields[0].value
      .split("\n")[1]
      .toLowerCase();
...
function hangman(answer: string, current: string, input: string) {
  return answer
    .split("")
    .map((char: string, index: number) => (char === input ? input : current[index]))
    .join("") === current
    ? false
    : current
        .split("")
        .map((char: string, index: number) => (answer[index] === input ? input : char))
        .join("");
}
