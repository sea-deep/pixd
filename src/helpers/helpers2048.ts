...
export function move(description: string, direction: string) {
...
export function spawnRandom(board: string[][], amount: number) {
  const emptyTiles = [];
...
export function makeDesc(board: string[][]) {
...
export function calculateScore(board: string[][]) {
  return board.flat().reduce((acc, v) => acc + (v !== "0" ? parseInt(v) : 0), 0);
}

export function isGameOver(board: string[][]) {
...
export function message2048(params: { description: string; score: number }): ContextReplyOptions {
  return {
    content: "",
    tts: false,
    components: [
      {
        type: 1,
        components: [
          { style: 2, custom_id: `empty1`, disabled: true, emoji: {  name: `🔴` }, type: 2 },
          { style: 1, custom_id: `2048up`, disabled: false, emoji: { id: `1088198768521908306`, name: `ArrowUp`, animated: false }, type: 2 },
          { style: 2, custom_id: `empty2`, disabled: true, emoji: {  name: `🔴` }, type: 2 },
        ],
      },
      {
        type: 1,
        components: [
          ...
          { style: 2, custom_id: `empty3`, disabled: true, emoji: {  name: `🔴` }, type: 2 },
      ...
      },
      {
        type: 1,
        components: [
          { style: 2, custom_id: `empty4`, disabled: true, emoji: {  name: `🔴` }, type: 2 },
          { style: 1, custom_id: `2048down`, disabled: false, emoji: { id: `1088199643382431836`, name: `ArrowDown`, animated: false }, type: 2 },
          { style: 2, custom_id: `empty5`, disabled: true, emoji: {  name: `🔴` }, type: 2 },
        ],
      },
    ],
    embeds: [
