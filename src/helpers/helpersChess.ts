...
import type { APIActionRowComponent, APIStringSelectComponent, APIButtonComponent } from "discord.js";
import sharp from "sharp";

export const pieceEmoji: Record<string, {name: string; id: string}> = {
...
export async function chess2img(board: ReturnType<Chess["board"]>, turn: Color) {
  let overlays = [
    {
...
export async function chessComponents(chess: Chess, turn: Color, selected?: string) {
  // const board = chess.board();
  let components: [APIActionRowComponent<APIStringSelectComponent>, APIActionRowComponent<APIStringSelectComponent>, APIActionRowComponent<APIButtonComponent>] = [
    {
      type: 1,
      components: [
        {
  ...
  if (selected) {
    ...
    let moves = chess.moves({
      square: selected.split("_")[0] as Square,
      verbose: true,
    });
    if (chess.turn() === "b") {
      moves = moves.reverse();
...
export async function getBotMove(fen: string, difficulty: "easy" | "medium" | "hard") {
  const depthMap = {
    easy: 5,
    medium: 10,
    hard: 15,
  ...
  try {
    ...
    if (data.success) {
      ...
      return { from, to, promotion: bestMoveString.slice(4) || undefined };
    } else {
...
export function incrementScore(content: string, player: Color, capturedPiece: string) {
