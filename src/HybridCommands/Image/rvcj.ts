  ...
  execute: async (ctx, client) => {
    ...
    try {
      ...
      for (const line of lines) {
        ...
        for (const token of tokens) {
          ...
          if (token === " ") {
            const spaceBuffer = await sharp({
              ...
              .resize({ height: 60, fit: "inside", withoutEnlargement: true })
              .png()
              .toBuffer();
            ...
            let textChar = await sharp({
              text: {
                text: escapeImageText(token.toUpperCase()),
                dpi: 400,
        ...
        textLine = await sharp(textLine).resize({ width: 1080, withoutEnlargement: true }).toBuffer();
...
const getEmojiImage = async (emoji: string) => {
