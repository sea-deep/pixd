...
export default new HybridCommand({
  ...
  execute: (ctx, client) => ctx.reply(buildHelp(client, {
    userId: ctx.user.id, inGuild: Boolean(ctx.guild), guildId: ctx.guild?.id || (ctx.raw as any)?.guildId, mode: ctx.isSlash ? "slash" : "prefix", nsfw: Boolean(ctx.channel && "nsfw" in ctx.channel && ctx.channel.nsfw),
